...
BotService.prototype.checkAvailability = function () {
  ...
  setInterval(function () {
    var tables = me.getAvailableTables();
    for (var table of tables) {
      var bot = me.getAvailableBot();
      if (bot && table && (!me.config.minBots || me.config.minBots > me.getActiveBots(table.id)) && !me.config.banBots) {
...
BotService.prototype.getActiveBots = function (tid) {
  var ctr = 0;
  for (var i in this.bots) {
    if (this.bots[i].tid === tid && !this.bots[i].available) {
      ctr += 1;
    }
  }
  return ctr;
...
BotService.prototype.getAvailableTables = function () {
  var tables = [];
  for (var i in this.tableService.tables) {
    if (!this.tableInstanceActive[i] && (this.tableService.tables[i].state == 'JOIN' && this.tableService.tables[i].table.playersToAdd.length < this.tableService.tables[i].table.maxPlayers) ||
      ...
      tables.push(this.tableService.tables[i]);
    }
  }
  return tables;
};
